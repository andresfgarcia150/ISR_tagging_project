...
\begin{itemize}
	...
	\item ROOT: Look at \texttt{/usr/include/root} \footnote{It depends on 
		your local installation directory. Another directory where it might
		be is \texttt{/usr/local/include/root/}}
...
\item Add the following libraries to the top right panel (Copy and paste them
on the panel):
...
are from ROOT and possibly, they are not the libraries installed on your PC. Type
\texttt{root-config --glibs} on terminal in order to display the available libraries.
