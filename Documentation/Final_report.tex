...
\begin{figure}[b!]
\centering
\includegraphics[width=0.54\linewidth]{./Imags_Doc/Eclipse_setting_2}
\caption[Setting the author in a eclipse project]{Setting the author in a eclipse project}
\label{fig:Eclipse_setting_2}
\end{figure}

\item In the last window (Fig. \ref{fig:Eclipse_setting_3}), select \texttt{Debug} 
and \texttt{Release}. Then click on finish.

\begin{figure}[h!]
	\centering
	\includegraphics[width=0.54\linewidth]{./Imags_Doc/Eclipse_setting_3}
...
\item After these settings, a basic Hello world template is created. On
the left menu, right-click on the project folder and choose the option 
\texttt{P\underline{r}operties}. It also can be done by typing 
\texttt{Alt + enter}.
...
\item On the right panel include the full path of the directories where
the .h files are located. To include the libraries of the following programs\footnote{ROOT is necessary to use the other two}:

\begin{itemize}
	\item ROOT: Use the the directory reported in the command \texttt{root-config
		 --cflags} of Fig. \ref{fig:root_cflags}.
	\item ExRootAnalysis: Search at the MadGraph directory and use the folder
	...
	a specific file in the code, write its relative location. Take a look, for
	instance, at the file \texttt{DelphesFunctions.h} of section \ref{sec:Root_makefile}. 
\end{itemize}

\item In the same menu of \texttt{GCC C++ Compiler}, go to \texttt{Miscellaneous} as illustrated
in Fig \ref{fig:Flags_eclipse}. Include the flags reported by the command of Fig.
\ref{fig:root_cflags} after the already existing flags in the label of the right. 

\begin{figure}[h]
\centering
\includegraphics[width=0.7\linewidth]{./Imags_Doc/Flags_eclipse}
\caption[Setting miscellaneous flags on eclipse]{Setting miscellaneous flags on eclipse}
\label{fig:Flags_eclipse}
\end{figure}
